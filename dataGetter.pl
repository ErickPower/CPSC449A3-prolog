...
getData([H|Text],_,RestOfText,Err):- H == "Name:",
    nth(Next,Text,"forced partial assignment:"),
    Len is Next-1,
    split(Len,Text,Emp,RestOfText),
    length(Emp,NoMoreOne),
    (NoMoreOne > 1 *-> Err is -1; Foo is 0).
